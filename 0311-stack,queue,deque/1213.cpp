...
vector<int> countAlphabet(string text) {
    ...
    for (int i = 0; i < text.size(); i++) { //알파벳의 개수만큼 반복함
        // 각 문자를 인덱스로 바꿔주기 위해 'A'의 인덱스를 빼줌
        count[text[i] - 'A']++; //해당하는 문자의 인덱스로 접근해 1을 더해줌
    }
    return count; //알파벳의 개수 리턴
}

string makePalindrome(vector<int> &count) {
    // 팰린드롬을 만들어 리턴하는 함수.
    string part1 = "", part2 = "", part3 = ""; //part1에는 중간 문자 이전 문자열 저장, part2는 중간 문자 저장, part3는 중간 문자 이후 문자열 저장
    ...
    for (int i = 0; i < SIZE; i++) {
        ...
        for (int j = 0; j < count[i] / 2; j++) {
            part1 += (char) (i + 'A'); //위에 countAlphabet 함수에서 빼준 A의 인덱스를 다시 더해준 후 추가해주어야함
            part3 = (char) (i + 'A') + part3;
        }
    }

    return part1 + part2 + part3; //완성된 팰린드롬 문자열 반환
}
