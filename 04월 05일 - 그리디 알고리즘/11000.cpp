...
int arrayRoom(int n, vector<ci> lec) {
    ...
    return pq.size(); //pq에 저장된 종료시간의 갯수 = 강의실 수 이므로 pq.size() return
}
...
int main() {
    ...
    //입력
    cin >> n; //강의 갯수 입력
    vector<ci> lec(n, ci(0, 0)); //강의 시작시간과 강의 종료시간 저장할 벡터
    for (int i = 0; i < n; i++) {
        cin >> lec[i].first >> lec[i].second; //first: 시작시간, second: 종료시간
    }

    //연산
    sort(lec.begin(), lec.end()); //정렬, 강의 시작시간 오름차순 강의 시작시간이 같으면 종료시간 오름차순
