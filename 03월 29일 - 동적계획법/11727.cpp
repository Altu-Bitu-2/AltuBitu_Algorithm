...
int fillTile(int n) {
    vector<int> dp(n + 1, 0);  //크기가 2Xn인 직사각형을 타일로 채우는 경우의 수를 저장할 벡터

    if (n == 1) { //인덱스 에러 안나도록 n이 1일 경우 바로 리턴
        //n이 1일 경우 아래 for문에서 인덱스 에러발생, n이 1일 경우 2x1타일로 채우는 경우의 수 밖에 없으므로 1 리턴
        return 1;
    }

    //너비 1, 2인 타일 채우는 경우의 수 미리 초기화
    dp[1] = 1; //n이 1일 경우 2x1타일로 채우는 경우의 수 하나
    dp[2] = 3; //n이 2일 경우 1x2 2개와 2x2 1개 총 2 경우 + 너비 1에 2x1 타일을 더한 1 경우 -> 3 경우
    for (int i = 3; i <= n; i++) {
